...
export function CommandDialog({ command, profileName, collapsed, dock, onChange, onConfirm, onConfirmDynamic, onCancel, onUndoSegment, onFinishPath, onToggleCollapsed }) {
  ...
  const title = isRectangle ? 'Prostokąt' : isCircle ? 'Okrąg' : isArc ? 'Łuk' : isPolygon ? 'Wielokąt regularny' : isEllipse ? 'Elipsa' : isSlot ? 'Slot' : isSpline ? 'Spline' : isConic ? 'Krzywa conic' : isPoint ? 'Punkt szkicu' : isSketch3D ? 'Szkic 3D' : isSketch3DEdit ? 'Edytuj krzywą 3D' : isProjectSketch ? (command.resumeSketch3D ? 'Pobierz krawędzie' : 'Rzutuj geometrię') : isExtrude ? 'Wyciągnięcie' : isSheetBase ? 'Baza blachowa' : isSheetFlange ? 'Kołnierz blachy' : isSheetHem ? 'Zawinięcie blachy' : isSheetRip ? 'Szczelina blachy' : isPlasticBoss ? 'Boss' : isSurfacePatch ? 'Patch powierzchni' : isSurfaceExtrude ? 'Wyciągnięcie powierzchni' : isSurfaceRevolve ? 'Obrót powierzchni' : isSurfaceSweep ? 'Powierzchnia po ścieżce' : isSurfaceLoft ? 'Powierzchnia przejściowa' : isSurfaceOffset ? 'Odsunięcie powierzchni' : isSurfaceStitch ? 'Zszyj powierzchnie' : isSurfaceTrim ? 'Przytnij powierzchnię' : isSurfaceExtend ? 'Przedłuż powierzchnię' : isThickenSurface ? 'Pogrub powierzchnię' : isRevolve ? 'Bryła obrotowa' : isSweep ? 'Przeciągnięcie po ścieżce' : isLoft ? 'Bryła przejściowa' : isRib ? 'Żebro' : isCoil ? 'Spirala' : isPipe ? 'Rura' : isPattern ? 'Szyk' : isBoolean ? 'Połącz / odejmij' : isPrimitive ? 'Prymityw 3D' : isTransform ? (command.mode === 'rotate' ? 'Obróć bryłę' : 'Przesuń bryłę') : isOffsetFace ? 'Odsuń ścianę' : isTextSolid ? 'Tekst 3D' : isHole ? 'Otwór' : isFillet ? 'Zaokrąglenie' : isShell ? 'Powłoka' : isDraftFeature ? 'Pochylenie ścian' : isSplitBody ? 'Podziel bryłę' : isSplitFace ? 'Podziel ścianę' : isDeleteFace ? 'Usuń i napraw ścianę' : isReplaceFace ? 'Zastąp ścianę' : command.type === 'line' ? 'Linia' : command.type === 'polyline' ? 'Polilinia' : isSketchMove ? 'Przesuń geometrię' : isSketchOffset ? 'Odsuń szkic' : isSketchCorner ? (command.mode === 'fillet' ? 'Zaokrąglij szkic' : 'Fazuj szkic') : isSketchTransform ? 'Transformuj szkic' : isSketchPattern ? 'Szyk szkicu' : isOffsetPlane ? 'Płaszczyzna odsunięta' : isMidplane ? 'Płaszczyzna środkowa' : isThreePointPlane ? 'Płaszczyzna przez trzy punkty' : isAnglePlane ? 'Płaszczyzna pod kątem' : isTangentPlane ? 'Płaszczyzna styczna' : isPathPlane ? 'Płaszczyzna na ścieżce' : isConstructionAxis ? axisTitles[command.axisType] : isConstructionPoint ? pointTitles[command.pointType] : 'Fazowanie';
  const dialogTitle = isProjectSurface ? 'Rzutuj na powierzchnię' : isPlasticSnapFit ? 'Zatrzask' : isPlasticGrille ? 'Kratka' : isFormBody ? 'Forma' : title;
  return (
