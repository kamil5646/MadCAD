    ...
    <aside className="measure-panel component-panel" aria-label="Komponenty i złożenia">
      ...
      <section className="component-storyboard" aria-label="Storyboard animacji złożenia">
        ...
        {activeStoryboard && <>
          ...
          <div className="storyboard-transport"><button type="button" aria-label={animationPlaying ? 'Zatrzymaj animację' : 'Odtwórz animację'} onClick={() => animationPlaying ? onStopStoryboard?.() : onPlayStoryboard?.(activeStoryboard)}>{animationPlaying ? <Pause size={14} /> : <Play size={14} />}{animationPlaying ? ' Stop' : ' Odtwórz'}</button><button type="button" disabled={readOnly} onClick={() => onAddStoryboardKeyframe?.(activeStoryboard.id, animationTime, explodeAmount, animationInstanceOffsets, animationNote)}><Plus size={13} /> Klatka</button></div>
          ...
          {selectedInstance && <div className="storyboard-instance-motion"><strong>Ruch: {selectedInstance.name}</strong><div>{['X', 'Y', 'Z'].map((axis, index) => <label key={axis}><span>{axis}</span><input aria-label={`Przesunięcie animacji ${axis}`} type="number" step="1" value={Number(selectedAnimationOffset[index].toFixed(2))} onChange={(event) => { const next = [...selectedAnimationOffset]; next[index] = Number(event.target.value) || 0; onPreviewStoryboardOffset?.(selectedInstance.id, next); }} /></label>)}</div></div>}
