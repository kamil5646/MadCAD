    ...
    <aside className="measure-panel component-panel" aria-label="Komponenty i złożenia">
      ...
      <section className="component-storyboard" aria-label="Storyboard animacji złożenia">
        ...
        {activeStoryboard && <>
          ...
          <div className="storyboard-transport"><button type="button" aria-label={animationPlaying ? 'Zatrzymaj animację' : 'Odtwórz animację'} onClick={() => animationPlaying ? onStopStoryboard?.() : onPlayStoryboard?.(activeStoryboard)}>{animationPlaying ? <Pause size={14} /> : <Play size={14} />}{animationPlaying ? ' Stop' : ' Odtwórz'}</button><button type="button" disabled={readOnly} onClick={() => onAddStoryboardKeyframe?.(activeStoryboard.id, animationTime, explodeAmount, animationInstanceOffsets, animationInstanceRotations, animationNote)}><Plus size={13} /> Klatka</button></div>
