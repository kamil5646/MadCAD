...
export default function ModelingWorkspace() {
  ...
  const openFormBody = () => {
    ...
    const next = { type: 'formBody', name: `Form ${sequence}`, width: '40', depth: '30', height: '20', subdivisions: '2', symmetry: 'none', controlOffsets: Array.from({ length: 8 }, () => ['0', '0', '0']), selectedControlKind: 'point', selectedControlPoint: 0, selectedControlEdge: 0, selectedControlFace: 0, creaseEdges: [], insertEdgeEnabled: false, insertEdgeIndex: 0, insertEdgePosition: '0.5', insertEdgeOffsets: Array.from({ length: 4 }, () => ['0', '0', '0']), x: '0', y: '0', z: '0', previewFeature: null };
    setCommand(next);
    window.setTimeout(() => updateCommand(next), 0);
        ...
        <main className="modeling-stage">
          ...
          <ModelViewport
            ...
            onFormControlPointMove={(selectedControlPoint, offset) => {
              const currentOffsets = [...Array.from({ length: 8 }, (_unused, index) => Array.from({ length: 3 }, (_axis, axis) => command?.controlOffsets?.[index]?.[axis] ?? '0')), ...(command?.insertEdgeEnabled ? Array.from({ length: 4 }, (_unused, index) => Array.from({ length: 3 }, (_axis, axis) => command?.insertEdgeOffsets?.[index]?.[axis] ?? '0')) : [])];
              const symmetryPairs = formControlSymmetryPairs({ enabled: command?.insertEdgeEnabled, edgeIndex: command?.insertEdgeIndex || 0, position: 0.5 }, command?.symmetry);
              ...
              updateCommand({ selectedControlPoint, controlOffsets: nextOffsets.slice(0, 8), insertEdgeOffsets: nextOffsets.slice(8) });
            }}
            onFormControlSelectionMove={(pointIndexes, axisIndex, delta) => {
              ...
              updateCommand({ controlOffsets: nextOffsets.slice(0, 8), insertEdgeOffsets: nextOffsets.slice(8) });
            }}
