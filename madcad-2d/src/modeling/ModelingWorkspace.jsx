  ...
  return (
    <section className="command-dialog" aria-label={title}>
      ...
      <div className="command-dialog-body">
        ...
        {(isExtrude || isRevolve || isSweep || (isHole && command.placement !== 'face-edges')) && <Field label="Profil" value={profileName} disabled />}
        {isExtrude && (
          <>
...
export default function ModelingWorkspace({ onClose }) {
  ...
  const actualBodyIds = useMemo(() => new Set(document.features.filter((feature) => (['extrude', 'revolve', 'sweep'].includes(feature.type) && feature.operation === 'new') || feature.type === 'primitive' || feature.type === 'importedModel' || feature.type === 'splitBody' || (feature.type === 'textSolid' && feature.operation === 'new')).map((feature) => `body-${feature.id}`)), [document.features]);
