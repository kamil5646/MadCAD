...
import {
  ...
  SketchCadIcon,
  SketchCircleCadIcon,
  SketchConstraintCadIcon,
  SketchDimensionCadIcon,
  SketchLineCadIcon,
  SketchPolylineCadIcon,
  SketchRectangleCadIcon,
  SketchShapesCadIcon,
  SketchTrimCadIcon,
  SplitBodyCadIcon,
  SplitFaceCadIcon,
  ...
  PlasticFeatureCadIcon,
  ProjectGeometryCadIcon,
  FinishSketchCadIcon,
...
export default function ModelingWorkspace() {
  ...
  useEffect(() => {
    ...
    window.__madcadVerifyEngineState = {
      ...
      evaluatedFeatureData: engine.evaluatedDocument?.features?.map((feature) => ({
        ...
        depth: feature.depth,
        counterboreDiameter: feature.counterboreDiameter,
        counterboreDepth: feature.counterboreDepth,
        countersinkDiameter: feature.countersinkDiameter,
        countersinkAngle: feature.countersinkAngle,
    ...
    setCommand((current) => {
      ...
      if (next.type === 'formBody') {
        next.previewFeature = createFeature('formBody', {
          ...
          fillHoleEnabled: next.fillHoleEnabled === true,
          fillHoleFace: next.fillHoleFace,
          fillHoleOffsets: next.fillHoleOffsets,
          x: next.x, y: next.y, z: next.z,
        });
        if (current.previewFeature?.id) next.previewFeature.id = current.previewFeature.id;
      }
  ...
  useEffect(() => {
    ...
    window.__madcadVerifyDocumentState = {
      ...
      command: command ? {
        ...
        fillHoleEnabled: command.fillHoleEnabled,
        fillHoleFace: command.fillHoleFace,
        fillHoleOffsets: command.fillHoleOffsets,
  ...
  const openFormBody = () => {
    ...
    const next = { type: 'formBody', name: `Form ${sequence}`, width: '40', depth: '30', height: '20', subdivisions: '2', symmetry: 'none', controlOffsets: Array.from({ length: 8 }, () => ['0', '0', '0']), selectedControlKind: 'point', selectedControlPoint: 0, selectedControlEdge: 0, selectedControlFace: 0, creaseEdges: [], insertEdgeEnabled: false, insertEdgeIndex: 0, insertEdgePosition: '0.5', insertEdgeOffsets: [], bridgeEnabled: false, bridgeFirstFace: 0, bridgeSecondFace: 1, bridgeInset: '0.45', bridgeOffsets: [], fillHoleEnabled: false, fillHoleFace: 0, fillHoleOffsets: [], x: '0', y: '0', z: '0', previewFeature: null };
    setCommand(next);
    window.setTimeout(() => updateCommand(next), 0);
        ...
        <div className="command-ribbon">
          ...
          <ResponsiveRibbon key={licenseInfoOpen ? 'license-open' : 'license-closed'} language={language}>
            {activeSketchId ? (
              ...
              <>
                <RibbonGroup label="ARKUSZ"><ToolButton icon={DrawingSheetCadIcon} label="Nowy arkusz" onClick={createDrawingSheetInDocument} disabled={readOnly} primary /><ToolMenuButton icon={FileText} label="Ustawienia" description="Format, tabliczka rysunkowa, rewizje i usuwanie arkusza." items={[
              ...
              <>
                <RibbonGroup label="WSTAW"><ToolButton icon={ImportMeshCadIcon} label="Importuj model" displayLabel="Importuj siatkę" onClick={() => { void requestModelImport(); }} primary disabled={readOnly || modelImportBusy} description="Wczytaj STL albo 3MF przez bezpieczne okno systemowe." /></RibbonGroup>
                <RibbonGroup label="PRZYGOTUJ">
                  <ToolButton icon={MeshBodyCadIcon} label="Narzędzia siatki" onClick={openMeshTools} disabled={readOnly || !selectedMeshFeature} disabledReason="Zaznacz zaimportowaną siatkę STL albo 3MF." />
                  <ToolButton icon={RotateCw} label="Przywróć siatkę" onClick={restoreSelectedBrepToMesh} disabled={readOnly || !selectedFacetedBrepFeature} disabledReason="Zaznacz model przekonwertowany do fasetowego B-Rep." />
                </RibbonGroup>
                <RibbonGroup label="WYJŚCIE">
                  <ToolButton icon={HardDriveDownload} label="STL" onClick={() => { void exportModel('stl'); }} disabled={!engine.bodies.length || engine.status !== 'ready'} />
                  <ToolButton icon={FileDown} label="3MF" onClick={() => { void exportModel('3mf'); }} disabled={!engine.bodies.length || engine.status !== 'ready'} />
                </RibbonGroup>
              </>
            ) : designTab === 'sheet' ? (
              <>
                <RibbonGroup label="UTWÓRZ">
                  <ToolButton icon={SheetMetalCadIcon} label="Baza blachowa" onClick={openSheetBase} primary disabled={readOnly || !selectedProfile || Boolean(activeSketchId)} disabledReason="Zaznacz zamknięty profil i zakończ szkic." />
                  <ToolButton icon={Layers3} label="Kołnierz blachy" onClick={openSheetFlange} disabled={readOnly || !canCreateSheetFlange || Boolean(activeSketchId)} disabledReason="Zaznacz prostą krawędź istniejącej blachy." />
                  <ToolButton icon={Layers3} label="Zawinięcie blachy" onClick={openSheetHem} disabled={readOnly || !canCreateSheetFlange || Boolean(activeSketchId)} disabledReason="Zaznacz prostą krawędź istniejącej blachy." />
                  <ToolButton icon={Scissors} label="Szczelina blachy" onClick={openSheetRip} disabled={readOnly || !canCreateSheetFlange || Boolean(activeSketchId)} disabledReason="Zaznacz prostą krawędź istniejącej blachy." />
                </RibbonGroup>
                <RibbonGroup label="ROZWINIĘCIE">
                  <ToolButton icon={Ungroup} label="Rozwiń blachę" onClick={() => addSheetStateFeature('sheetUnfold')} disabled={readOnly || !canUnfoldSheet || Boolean(activeSketchId)} disabledReason="Zaznacz blachę z gięciem." />
                  <ToolButton icon={Layers3} label="Zagnij ponownie" onClick={() => addSheetStateFeature('sheetRefold')} disabled={readOnly || !canRefoldSheet || Boolean(activeSketchId)} disabledReason="Najpierw rozwiń blachę." />
                </RibbonGroup>
              </>
            ) : designTab === 'plastic' ? (
              <>
              ...
              <>
                <RibbonGroup label="SPRAWDŹ">
                  <ToolButton icon={Ruler} label="Zmierz" onClick={openMeasure} primary />
                  <ToolButton icon={SectionCadIcon} label="Przekrój" onClick={openSectionAnalysis} disabled={!engine.bodies.length} />
                  <ToolButton icon={ScanSearch} label="Analiza powierzchni" onClick={openSurfaceAnalysis} disabled={!engine.bodies.length} />
                  <ToolButton icon={MassCadIcon} label="Właściwości masy" onClick={openMassProperties} disabled={!engine.bodies.length} />
        ...
        <main className="modeling-stage">
          ...
          <ModelViewport
            ...
            onFormControlPointMove={(selectedControlPoint, offset) => {
              ...
              const bridgeCount = command?.bridgeEnabled ? 8 : 0;
              const fillHoleCount = command?.fillHoleEnabled ? Math.max(1, command?.fillHoleOffsets?.length || 0) : 0;
              const currentOffsets = [...Array.from({ length: 8 }, (_unused, index) => Array.from({ length: 3 }, (_axis, axis) => command?.controlOffsets?.[index]?.[axis] ?? '0')), ...(insertCount ? Array.from({ length: 4 }, (_unused, index) => Array.from({ length: 3 }, (_axis, axis) => command?.insertEdgeOffsets?.[index]?.[axis] ?? '0')) : []), ...(bridgeCount ? Array.from({ length: 8 }, (_unused, index) => Array.from({ length: 3 }, (_axis, axis) => command?.bridgeOffsets?.[index]?.[axis] ?? '0')) : []), ...(fillHoleCount ? Array.from({ length: fillHoleCount }, (_unused, index) => Array.from({ length: 3 }, (_axis, axis) => command?.fillHoleOffsets?.[index]?.[axis] ?? '0')) : [])];
              const symmetryPairs = formControlSymmetryPairs({ enabled: command?.insertEdgeEnabled, edgeIndex: command?.insertEdgeIndex || 0, position: Number(command?.insertEdgePosition) || 0.5 }, command?.symmetry, { enabled: command?.bridgeEnabled, firstFaceIndex: command?.bridgeFirstFace || 0, secondFaceIndex: command?.bridgeSecondFace ?? 1, inset: Number(command?.bridgeInset) || 0.45 }, { enabled: command?.fillHoleEnabled, faceIndex: command?.fillHoleFace || 0 });
              ...
              updateCommand({ selectedControlPoint, controlOffsets: nextOffsets.slice(0, 8), insertEdgeOffsets: nextOffsets.slice(8, 8 + insertCount), bridgeOffsets: nextOffsets.slice(8 + insertCount, 8 + insertCount + bridgeCount), fillHoleOffsets: nextOffsets.slice(8 + insertCount + bridgeCount) });
            }}
            onFormControlSelectionMove={(pointIndexes, axisIndex, delta) => {
              ...
              updateCommand({ controlOffsets: nextOffsets.slice(0, 8), insertEdgeOffsets: nextOffsets.slice(8, 8 + insertCount), bridgeOffsets: nextOffsets.slice(8 + insertCount, 8 + insertCount + bridgeCount), fillHoleOffsets: nextOffsets.slice(8 + insertCount + bridgeCount) });
            }}
