...
export default function ModelingWorkspace() {
  ...
  const openBeamFea = () => {
    ...
    setCommand({ type: 'beamFea', bodyId: body.id, materialId: 's235', spanAxis: ['x', 'y', 'z'][spanIndex], loadAxis: ['x', 'y', 'z'][loadIndex], loadType: 'tip', loadPositionPercent: '100', force: '1000', elementCount: '8' });
    ...
    <section className={`modeling-shell platform-${DESKTOP_PLATFORM} ${workspace === 'drawing' ? 'drawing-mode' : workspace === 'tools' ? 'tools-mode' : activeSketchId ? 'sketch-mode' : document.features.length ? '' : 'timeline-empty'} ${startPageVisible ? 'start-page-mode' : ''}`} aria-label="Modelowanie parametryczne MadCAD">
      ...
      >
        ...
        <main className="modeling-stage">
          ...
          {command?.type === 'beamFea' && <BeamFeaPanel bodies={engine.bodies.filter((body) => body.bodyKind !== 'surface')} bodyId={command.bodyId} materialId={command.materialId} spanAxis={command.spanAxis} loadAxis={command.loadAxis} loadType={command.loadType} loadPositionPercent={command.loadPositionPercent} force={command.force} elementCount={command.elementCount} result={beamFea?.result} error={beamFea?.error} onChange={(patch) => setCommand((current) => ({ ...current, ...patch }))} onClose={() => setCommand(null)} />}
