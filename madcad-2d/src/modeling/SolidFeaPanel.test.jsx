...
describe('SolidFeaPanel', () => {
  it('shows mesh quality, equilibrium and the 3D result without presenting beta as certified', () => {
    const onChange = vi.fn();
    render(<SolidFeaPanel bodies={[{ id: 'body-1', name: 'Wspornik' }]} bodyId="body-1" result={{ status: 'safe', nodeCount: 63, elementCount: 144, fixedNodeCount: 9, loadedNodeCount: 9, maximumDisplacement: 0.01234, maximumStress: 42.5, safetyFactor: 5.52, volumeErrorPercent: 1.25, equilibriumErrorPercent: 0.0001, iterationCount: 87, limitations: ['Etap beta.'] }} onChange={onChange} onClose={vi.fn()} />);
    expect(screen.getByRole('complementary', { name: /MES bryły 3D beta/i })).toBeInTheDocument();
    ...
    expect(screen.getByText('Etap beta.')).toBeInTheDocument();
