...
import { AlertTriangle, Box, CheckCircle2, Clock3, Code2, Crosshair, FileDown, Flame, Gauge, Layers3, Pause, Play, Plus, RotateCcw, Route, ScanLine, ShieldCheck, Trash2, X } from 'lucide-react';
...
export function ManufacturingPanel({ manufacturing, bodies = [], projectDocument = null, simulationProgress = 1, onSimulationProgress = () => {}, readOnly = false, onCreate, onActivate, onUpdate, onDelete, onCreateOperation, onUpdateOperation, onDeleteOperation, onExportOperation }) {
  ...
  return (
    <aside className="manufacturing-panel" aria-label="Setup wytwarzania CAM">
      <header>
        <div>{isCuttingSetup ? <Flame size={18} /> : <Box size={18} />}<span><strong>Setup CAM</strong><small>{isCuttingSetup ? 'Cięcie laserowe / plazmowe 2D' : 'Frezowanie 3-osiowe'}</small></span></div>
      ...
      {!solidBodies.length ? <div className="manufacturing-empty"><AlertTriangle size={22} /><strong>Najpierw utwórz bryłę</strong><p>Setup wymaga gotowego modelu 3D. Wróć do Projektuj, utwórz albo zaimportuj bryłę i uruchom Wytwarzanie ponownie.</p></div> : !setup ? <div className="manufacturing-empty"><Box size={26} /><strong>Przygotuj pierwszą obróbkę</strong><p>Setup łączy model z obrabiarką, półfabrykatem i układem współrzędnych.</p><button type="button" disabled={readOnly} onClick={onCreate}><Plus size={15} /> Utwórz Setup</button></div> : <>
        ...
        <div className="manufacturing-form">
          ...
          <label><span>Rodzaj obróbki</span><select value={setup.operationKind} disabled={readOnly || setup.operations.length > 0} onChange={(event) => update({ machineId: event.target.value === 'cut-2d' ? 'laser-600' : 'mill-500' })}><option value="mill-3axis">Frezowanie 3-osiowe</option><option value="cut-2d">Cięcie 2D</option></select></label>
        ...
        {panelPage === 'operations' && <section className="manufacturing-operation-list" aria-label="Operacje CAM">
          ...
          {setup.operations.map((operation, operationIndex) => {
            ...
            const OperationIcon = isCut2d ? Flame : isContour ? Route : isPocket ? Layers3 : isAdaptive ? Gauge : ScanLine;
            return <section className="manufacturing-operation" key={operation.id}>
              <header><div><OperationIcon size={15} /><span><strong>{operationIndex + 1} · {operation.name}</strong><small>{isCut2d ? 'Skompensowany obrys z wejściem i sterowaniem źródłem' : isContour ? 'Zewnętrzny obrys górnej powierzchni' : isPocket ? 'Wnętrze górnego obrysu bryły' : isAdaptive ? 'Stałe obciążenie i wejście rampą' : 'Równoległe planowanie powierzchni'}</small></span></div><button type="button" aria-label={`Usuń operację ${operation.name}`} disabled={readOnly} onClick={() => onDeleteOperation(setup.id, operation.id)}><Trash2 size={13} /></button></header>
              <div className="manufacturing-form operation-form">
                ...
                {!isCut2d && <label><span>Narzędzie</span><select value={operation.toolId} disabled={readOnly} onChange={(event) => onUpdateOperation(setup.id, operation.id, { toolId: event.target.value })}>{Object.values(CAM_TOOL_PRESETS).map((tool) => <option value={tool.id} key={tool.id}>{tool.name}</option>)}</select></label>}
                <label><span>Sterownik / postprocesor</span><select value={operation.postProcessorId} disabled={readOnly || isCut2d} onChange={(event) => onUpdateOperation(setup.id, operation.id, { postProcessorId: event.target.value })}>{Object.values(CAM_POST_PROCESSORS).filter((post) => isCut2d ? post.id === (setup.machineId === 'plasma-1250' ? 'linuxcnc-plasma' : 'grbl-laser') : !['grbl-laser', 'linuxcnc-plasma'].includes(post.id)).map((post) => <option value={post.id} key={post.id}>{post.name} (.{post.extension})</option>)}</select></label>
                <div className="manufacturing-field-grid">
                  {isCut2d ? <><label><span>Szczelina</span><input type="number" min="0.01" step="0.05" value={operation.kerfWidth} disabled={readOnly} onChange={(event) => onUpdateOperation(setup.id, operation.id, { kerfWidth: event.target.value })} /><em>mm</em></label><label><span>Wejście</span><input type="number" min="0" step="0.5" value={operation.leadIn} disabled={readOnly} onChange={(event) => onUpdateOperation(setup.id, operation.id, { leadIn: event.target.value })} /><em>mm</em></label><label><span>Moc</span><input type="number" min="1" max="100" step="1" value={operation.powerPercent} disabled={readOnly} onChange={(event) => onUpdateOperation(setup.id, operation.id, { powerPercent: event.target.value })} /><em>%</em></label><label><span>Przejścia</span><input type="number" min="1" max="100" step="1" value={operation.passes} disabled={readOnly} onChange={(event) => onUpdateOperation(setup.id, operation.id, { passes: event.target.value })} /></label></> : isContour || isPocket || isAdaptive ? <label><span>Głębokość</span><input type="number" min="0.05" step="0.1" value={operation.targetDepth} disabled={readOnly} onChange={(event) => onUpdateOperation(setup.id, operation.id, { targetDepth: event.target.value })} /><em>mm</em></label> : <label><span>Stepover</span><input type="number" min="0.1" max="0.9" step="0.05" value={operation.stepover} disabled={readOnly} onChange={(event) => onUpdateOperation(setup.id, operation.id, { stepover: event.target.value })} /><em>×D</em></label>}
                  ...
                  {!isCut2d && <label><span>Zejście</span><input type="number" min="0.05" step="0.1" value={operation.maxStepdown} disabled={readOnly} onChange={(event) => onUpdateOperation(setup.id, operation.id, { maxStepdown: event.target.value })} /><em>mm</em></label>}
