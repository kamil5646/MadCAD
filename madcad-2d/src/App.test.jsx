...
describe('App', () => {
  ...
  it('requires a server-backed account even for the free personal plan', () => {
    ...
    expect(dialog).toHaveTextContent(/Wydanie 6.5.20 nie ma podpisu producenta/i);
  ...
  it('announces the update result, moves focus inside and closes with Escape', async () => {
    ...
    const { unmount } = render(<UpdateDialog
      state={{ status: 'idle', error: '', result: { available: false, currentVersion: '6.4.7' } }}
      onCheck={() => {}}
      onInstall={() => {}}
      onClose={onClose}
    />);
    ...
    expect(screen.getByRole('status')).toHaveTextContent(/Masz aktualną wersję MadCAD \(6.4.7\)/i);
    fireEvent.keyDown(window, { key: 'Escape' });
    expect(onClose).toHaveBeenCalledOnce();
    unmount();
