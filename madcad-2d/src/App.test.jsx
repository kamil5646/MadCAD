...
describe('App', () => {
  ...
  it('shows the private-use, evaluation, and commercial-license terms without a key field', () => {
    ...
    expect(dialog).toHaveTextContent(/Wydanie 6.1.8 nie ma podpisu producenta/i);
  ...
  it('links to the release when a newer version has no compatible package', () => {
    render(<UpdateDialog
      state={{ status: 'idle', error: '', result: { newerVersion: true, supported: false, currentVersion: '6.1.8', latestVersion: '6.2.0', releaseUrl: 'https://github.com/kamil5646/MadCAD2D/releases/tag/v6.2.0' } }}
      onCheck={() => {}}
      onInstall={() => {}}
      onClose={() => {}}
    />);
  ...
  it('confirms that a verified update package was opened for installation', () => {
    render(<UpdateDialog
      state={{ status: 'idle', error: '', result: { available: true }, handoff: { latestVersion: '6.1.8', opened: true, downloadedPath: '/Downloads/MadCAD.zip' } }}
      onCheck={() => {}}
      onInstall={() => {}}
      onClose={() => {}}
    />);
    expect(screen.getByText(/Paczka wersji 6.1.8 jest gotowa/i)).toBeInTheDocument();
