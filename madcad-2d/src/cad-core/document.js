...
export const DOCUMENT_SCHEMA_VERSION = 20;
...
export function validateDocument(document) {
  ...
  features.forEach((feature, featureIndex) => {
    ...
    if (feature.type === 'plasticBoss') {
      if (!bodyIds.has(feature.targetBodyId) || surfaceBodyIds.has(feature.targetBodyId)) add(`${base}.targetBodyId`, 'Boss wymaga istniejącej bryły B-Rep.', 'BROKEN_REFERENCE');
      if (!Array.isArray(feature.referenceIds) || feature.referenceIds.length !== 1) add(`${base}.referenceIds`, 'Boss wymaga dokładnie jednej planarnej ściany.', 'REQUIRED');
      else {
        const faceReference = references.find((reference) => reference.id === feature.referenceIds[0]);
        if (faceReference?.kind !== 'topology' || faceReference.topologyKind !== 'face' || faceReference.bodyId !== feature.targetBodyId || faceReference.descriptor?.geometry !== 'PLANE') add(`${base}.referenceIds[0]`, 'Boss wymaga trwałej referencji planarnej ściany wybranej bryły.', 'UNSUPPORTED');
      }
      for (const field of ['outerDiameter', 'holeDiameter', 'height', 'holeDepth', 'offsetX', 'offsetY']) {
        if (typeof feature[field] !== 'string' && typeof feature[field] !== 'number') add(`${base}.${field}`, `Boss wymaga parametrycznej wartości ${field}.`, 'TYPE');
      }
      if (typeof feature.reverse !== 'boolean') add(`${base}.reverse`, 'Kierunek Boss musi być wartością logiczną.', 'TYPE');
    }

    if (feature.type === 'plasticSnapFit') {
      if (!bodyIds.has(feature.targetBodyId) || surfaceBodyIds.has(feature.targetBodyId)) add(`${base}.targetBodyId`, 'Snap-fit wymaga istniejącej bryły B-Rep.', 'BROKEN_REFERENCE');
      if (!Array.isArray(feature.referenceIds) || feature.referenceIds.length !== 1) add(`${base}.referenceIds`, 'Snap-fit wymaga dokładnie jednej planarnej ściany.', 'REQUIRED');
      else {
        const faceReference = references.find((reference) => reference.id === feature.referenceIds[0]);
        if (faceReference?.kind !== 'topology' || faceReference.topologyKind !== 'face' || faceReference.bodyId !== feature.targetBodyId || faceReference.descriptor?.geometry !== 'PLANE') add(`${base}.referenceIds[0]`, 'Snap-fit wymaga trwałej referencji planarnej ściany wybranej bryły.', 'UNSUPPORTED');
      }
      for (const field of ['length', 'width', 'thickness', 'clearance', 'hookLength', 'hookHeight', 'offsetX', 'offsetY']) {
        if (typeof feature[field] !== 'string' && typeof feature[field] !== 'number') add(`${base}.${field}`, `Snap-fit wymaga parametrycznej wartości ${field}.`, 'TYPE');
      }
      if (typeof feature.reverse !== 'boolean') add(`${base}.reverse`, 'Kierunek Snap-fit musi być wartością logiczną.', 'TYPE');
    }

    if (feature.type === 'plasticGrille') {
      if (!bodyIds.has(feature.targetBodyId) || surfaceBodyIds.has(feature.targetBodyId)) add(`${base}.targetBodyId`, 'Grille wymaga istniejącej bryły B-Rep.', 'BROKEN_REFERENCE');
      if (!Array.isArray(feature.referenceIds) || feature.referenceIds.length !== 1) add(`${base}.referenceIds`, 'Grille wymaga dokładnie jednej planarnej ściany.', 'REQUIRED');
      else {
        const faceReference = references.find((reference) => reference.id === feature.referenceIds[0]);
        if (faceReference?.kind !== 'topology' || faceReference.topologyKind !== 'face' || faceReference.bodyId !== feature.targetBodyId || faceReference.descriptor?.geometry !== 'PLANE') add(`${base}.referenceIds[0]`, 'Grille wymaga trwałej referencji planarnej ściany wybranej bryły.', 'UNSUPPORTED');
      }
      for (const field of ['ribCount', 'ribWidth', 'gap', 'length', 'depth', 'offsetX', 'offsetY']) {
        if (typeof feature[field] !== 'string' && typeof feature[field] !== 'number') add(`${base}.${field}`, `Grille wymaga parametrycznej wartości ${field}.`, 'TYPE');
      }
      if (typeof feature.reverse !== 'boolean') add(`${base}.reverse`, 'Kierunek Grille musi być wartością logiczną.', 'TYPE');
    }
    ...
    if (feature.type === 'formBody') {
      ...
      if (feature.insertEdgeEnabled !== undefined && typeof feature.insertEdgeEnabled !== 'boolean') add(`${base}.insertEdgeEnabled`, 'Insert Edge wymaga wartości logicznej.', 'TYPE');
