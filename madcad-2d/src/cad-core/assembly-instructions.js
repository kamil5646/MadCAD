...
export function assemblyInstructionHtml(document, storyboard) {
  ...
  const rows = [...storyboard.keyframes]
    ...
    .map((frame, index) => {
      ...
      return `<tr><td>${index + 1}</td><td>${Number(frame.time).toFixed(1)} s</td><td>${Math.round(Number(frame.explodeAmount || 0) * 100)}%</td><td><strong>${escapeHtml(frame.note || (index ? `Krok ${index + 1}` : 'Położenie początkowe'))}</strong>${details.length ? `<ul>${details.map((detail) => `<li>${detail}</li>`).join('')}</ul>` : ''}</td></tr>`;
    }).join('');
  return `<!doctype html>
<html lang="pl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>${escapeHtml(document.name)} — ${escapeHtml(storyboard.name)}</title>
<style>body{font:14px/1.45 system-ui,sans-serif;color:#17212b;max-width:960px;margin:32px auto;padding:0 24px}header{border-bottom:3px solid #1596bd;margin-bottom:24px}h1{margin:0 0 4px;font-size:25px}p{color:#52616d}table{width:100%;border-collapse:collapse}th,td{border:1px solid #b8c3cb;padding:9px;text-align:left;vertical-align:top}th{background:#eaf2f5}td:first-child{font-weight:700;text-align:center}ul{margin:7px 0 0;padding-left:20px}@media print{body{margin:0;max-width:none}thead{display:table-header-group}}</style></head>
...
<table><thead><tr><th>Krok</th><th>Czas</th><th>Rozłożenie</th><th>Instrukcja i stan złożenia</th></tr></thead><tbody>${rows}</tbody></table></body></html>`;
}
