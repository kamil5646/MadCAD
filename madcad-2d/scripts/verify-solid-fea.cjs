...
app.whenReady().then(async () => {
  ...
  try {
    ...
    await waitFor(window, `document.querySelector('[data-tool-label="MES bryły 3D"]')`, 'polecenie MES bryły 3D');
    await window.webContents.executeJavaScript(`document.querySelector('[data-tool-label="MES bryły 3D"]').click()`);
