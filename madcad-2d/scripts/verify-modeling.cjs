...
async function runUiFlow(window) {
  ...
  const solidToolMenus = new Map([
    ...['Baza blachowa', 'Kołnierz blachy'].map((label) => [label, 'Blacha']),
  ...
  await sendMouse('mouseDown', referenceSnapPointer);
  await sendMouse('mouseUp', referenceSnapPointer);
  await waitForUi(window, `(() => { const point = window.__madcadVerifyDocumentState?.sketches?.at(-1)?.entityData?.find((entity) => entity.type === 'point'); return window.__madcadVerifyDocumentState?.sketches?.at(-1)?.entities === 1 && Number(point?.geometry?.x) === -20 && Number(point?.geometry?.y) === -10; })()`, 'nowy punkt osadzony dokładnie na końcu wcześniejszego szkicu');
  ...
  await sendMouse('mouseDown', secondReferenceSnapPointer);
  await sendMouse('mouseUp', secondReferenceSnapPointer);
  await waitForUi(window, `(() => { const entities = window.__madcadVerifyDocumentState?.sketches?.at(-1)?.entityData || []; const points = entities.filter((entity) => entity.type === 'point'); return entities.length === 3 && points.some((point) => Number(point.geometry?.x) === -20 && Number(point.geometry?.y) === -10) && points.some((point) => Number(point.geometry?.x) === 20 && Number(point.geometry?.y) === 10) && !document.querySelector('.command-dialog'); })()`, 'nowa linia połączona dokładnie z oboma końcami wcześniejszego szkicu');
