  ...
  await waitForUi(
    ...
    `(() => {
      try {
        ...
        return saved?.schemaVersion === 7 && saved?.features?.length === 5 && saved?.sketches?.length === 4 && saved?.references?.some((item) => item.kind === 'construction-plane' && item.name === 'Płaszczyzna montażowa');
      } catch (_error) {
        return false;
      }
    })()`,
