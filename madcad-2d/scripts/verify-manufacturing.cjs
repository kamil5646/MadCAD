...
app.whenReady().then(async () => {
  ...
  try {
    ...
    await waitFor(window, `document.querySelector('.manufacturing-gcode-preview pre')?.textContent.includes('Wiertło kręte Ø8') && document.querySelector('.manufacturing-gcode-preview pre')?.textContent.includes('\\nG1 X0 Y0 Z-')`, 'G-code wiercenia po skokach');
    ...
    if (contourDiagnostic.summaries.length !== 5 || !contourDiagnostic.summaries[4].valid || contourDiagnostic.drilling.type !== 'drill' || contourDiagnostic.drilling.toolId !== 'drill-8' || contourDiagnostic.body.holeInstances < 1) throw new Error(`Kontur lub wiercenie nie jest prawidłowe: ${JSON.stringify(contourDiagnostic)}`);
    ...
    if (state.schemaVersion !== DOCUMENT_SCHEMA_VERSION || !state.setup?.id || state.activeSetupId !== state.setup.id || state.setup.bodyId !== selectedBoundary.bodyId || state.setup.stock.topOffset !== 5 || state.setup.operations?.length !== 5 || state.setup.operations[1].type !== 'pocket' || state.setup.operations[1].boundaryFaceId !== selectedBoundary.faceId || state.setup.operations[2].type !== 'adaptive' || state.setup.operations[2].boundaryFaceId !== selectedBoundary.faceId || state.setup.operations[3].type !== 'drill' || state.setup.operations[4].type !== 'contour' || state.setup.operations[4].boundaryFaceId !== selectedBoundary.faceId || state.setup.operations[4].postProcessorId !== 'linuxcnc' || !state.valid || state.validToolpaths !== 5 || !(state.toolpathSegments > 0) || !state.stockVisible || !state.insideViewport || state.horizontalOverflow) throw new Error(`Niepoprawny Setup CAM: ${JSON.stringify(state)}`);
