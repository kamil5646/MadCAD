...
app.whenReady().then(async () => {
  ...
  try {
    ...
    await waitFor(window, `JSON.parse(window.__madcadGetSessionExport()).manufacturing.setups[0].fixtures[1].name === 'Szczęka prawa' && JSON.parse(window.__madcadGetSessionExport()).manufacturing.setups[0].fixtures[1].bounds[0][0] === 80 && JSON.parse(window.__madcadGetSessionExport()).manufacturing.setups[0].fixtures[1].bounds[1][0] === 100`, 'edycja wymiarów i nazwy uchwytu');
    ...
    await waitFor(window, `JSON.parse(window.__madcadGetSessionExport()).manufacturing.setups[0].fixtures[1].enabled && window.__madcadManufacturingVisualState?.fixtureCount === 2`, 'wizualizacja wielu uchwytów CAM');
    await window.webContents.executeJavaScript(`document.querySelector('#undoProjectBtn').click()`);
