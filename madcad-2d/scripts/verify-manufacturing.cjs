...
app.whenReady().then(async () => {
  ...
  try {
    ...
    if (state.schemaVersion !== 17 || !state.setup?.id || state.activeSetupId !== state.setup.id || state.setup.bodyId !== selectedBoundary.bodyId || state.setup.stock.topOffset !== 5 || state.setup.operations?.length !== 3 || state.setup.operations[1].type !== 'pocket' || state.setup.operations[1].boundaryFaceId !== selectedBoundary.faceId || state.setup.operations[2].type !== 'contour' || state.setup.operations[2].boundaryFaceId !== selectedBoundary.faceId || !state.valid || state.validToolpaths !== 3 || !(state.toolpathSegments > 0) || !state.stockVisible || !state.insideViewport || state.horizontalOverflow) throw new Error(`Niepoprawny Setup CAM: ${JSON.stringify(state)}`);
    ...
    await waitFor(window, `JSON.parse(window.__madcadGetSessionExport()).manufacturing.setups[0].operations.length === 2`, 'Cofnij dodanie Konturu 2D');
