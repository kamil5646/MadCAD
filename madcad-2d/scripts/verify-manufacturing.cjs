...
app.whenReady().then(async () => {
  ...
  try {
    ...
    await window.webContents.executeJavaScript(`(() => { const control = document.querySelectorAll('.manufacturing-operation')[3].querySelectorAll('select')[2]; Object.getOwnPropertyDescriptor(HTMLSelectElement.prototype, 'value').set.call(control, ${JSON.stringify(customToolId)}); control.dispatchEvent(new Event('change', { bubbles: true })); })()`);
    ...
    await waitFor(window, `window.__madcadVerifyEngineState?.status === 'ready' && JSON.parse(window.__madcadGetSessionExport()).manufacturing.setups[0].operations[3]?.type === 'drill'`, 'ponowne otwarcie projektu z wierceniem');
    await window.webContents.executeJavaScript(`[...document.querySelectorAll('.workspace-tabs button')].find((button) => button.textContent.trim() === 'WYTWARZANIE').click()`);
    ...
    await waitFor(window, `document.querySelector('.manufacturing-program-report > header.valid') && document.querySelectorAll('.manufacturing-report-operations > div.valid').length === 5`, 'raport bezpieczeństwa i czasu CAM');
