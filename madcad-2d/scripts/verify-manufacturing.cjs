...
const fixtureBodyScreenshotPath = path.join(__dirname, '..', 'artifacts', 'madcad-manufacturing-fixture-body.png');
...
app.whenReady().then(async () => {
  ...
  try {
    ...
    await setControl(window, '.manufacturing-operation [aria-label="Średnica szyjki oprawki"]', '9');
    await setControl(window, '.manufacturing-operation [aria-label="Długość szyjki oprawki"]', '6');
    await waitFor(window, `JSON.parse(window.__madcadGetSessionExport()).manufacturing.tools[0].holderNeckDiameter === 9 && JSON.parse(window.__madcadGetSessionExport()).manufacturing.tools[0].holderNeckLength === 6`, 'stopniowany profil oprawki');
    await window.webContents.executeJavaScript(`document.querySelector('#undoProjectBtn').click()`);
    await waitFor(window, `JSON.parse(window.__madcadGetSessionExport()).manufacturing.tools[0].holderNeckLength === 0`, 'Cofnij długość szyjki oprawki');
    await window.webContents.executeJavaScript(`document.querySelector('#redoProjectBtn').click()`);
    await waitFor(window, `JSON.parse(window.__madcadGetSessionExport()).manufacturing.tools[0].holderNeckLength === 6`, 'Ponów długość szyjki oprawki');
    await window.webContents.executeJavaScript(`[...document.querySelectorAll('button')].find((button) => button.textContent.includes('Dodaj narzędzie')).click()`);
    ...
    await waitFor(window, `window.__madcadVerifyEngineState?.status === 'ready' && JSON.parse(window.__madcadGetSessionExport()).manufacturing.setups[0].operations[3]?.type === 'drill' && JSON.parse(window.__madcadGetSessionExport()).manufacturing.setups[0].operationGroups[0]?.id === ${JSON.stringify(operationGroupId)} && JSON.parse(window.__madcadGetSessionExport()).manufacturing.operationTemplates.length === 1 && JSON.parse(window.__madcadGetSessionExport()).manufacturing.tools[0]?.holderNeckLength === 6`, 'ponowne otwarcie projektu z folderem, szablonem CAM i oprawką');
    await window.webContents.executeJavaScript(`[...document.querySelectorAll('.workspace-tabs button')].find((button) => button.textContent.trim() === 'WYTWARZANIE').click()`);
    ...
    if (!setupSheet.includes('<h1>Arkusz ustawczy CAM</h1>') || !setupSheet.includes('Półfabrykat X × Y × Z') || !setupSheet.includes('<span>Układ roboczy</span><strong>G55</strong>') || !setupSheet.includes('Wiertło własne 1') || !setupSheet.includes('Planowanie 1') || !setupSheet.includes('Adaptacyjne 2D 1') || !setupSheet.includes('Kieszeń 2D 2') || !setupSheet.includes('ŚCIEŻKI SPRAWDZONE W MODELU')) throw new Error('Arkusz ustawczy CAM nie zawiera kompletnych danych produkcyjnych.');
    ...
    await window.webContents.executeJavaScript(`(() => { const control = document.querySelectorAll('.manufacturing-fixture')[1].querySelector('select'); Object.getOwnPropertyDescriptor(HTMLSelectElement.prototype, 'value').set.call(control, 'body'); control.dispatchEvent(new Event('change', { bubbles: true })); })()`);
    await waitFor(window, `(() => { const setup = JSON.parse(window.__madcadGetSessionExport()).manufacturing.setups[0]; return setup.fixtures[1].shape === 'body' && setup.fixtures[1].bodyId && setup.fixtures[1].bodyId !== setup.bodyId && window.__madcadManufacturingVisualState?.fixtureShapes?.[1] === 'body' && window.__madcadManufacturingVisualState?.fixtureBodyMeshCount === 1; })()`, 'szczęka z bryły CAD w projekcie i widoku');
    await window.webContents.executeJavaScript(`document.querySelectorAll('.manufacturing-fixture')[1].scrollIntoView({ block: 'center' })`);
    await new Promise((resolve) => setTimeout(resolve, 150));
    await fs.writeFile(fixtureBodyScreenshotPath, (await window.webContents.capturePage()).toPNG());
    await window.webContents.executeJavaScript(`document.querySelector('#undoProjectBtn').click()`);
    await waitFor(window, `JSON.parse(window.__madcadGetSessionExport()).manufacturing.setups[0].fixtures[1].shape === 'cylinder' && window.__madcadManufacturingVisualState?.fixtureBodyMeshCount === 0`, 'Cofnij szczękę z bryły CAD');
    await window.webContents.executeJavaScript(`document.querySelector('#redoProjectBtn').click()`);
    await waitFor(window, `JSON.parse(window.__madcadGetSessionExport()).manufacturing.setups[0].fixtures[1].shape === 'body' && window.__madcadManufacturingVisualState?.fixtureBodyMeshCount === 1`, 'Ponów szczękę z bryły CAD');
    await window.webContents.executeJavaScript(`document.querySelector('#undoProjectBtn').click()`);
    await waitFor(window, `JSON.parse(window.__madcadGetSessionExport()).manufacturing.setups[0].fixtures[1].shape === 'cylinder' && window.__madcadManufacturingVisualState?.fixtureBodyMeshCount === 0`, 'Przywróć walcową szczękę CAM');
    ...
    await window.webContents.executeJavaScript(`(() => { const control = document.querySelectorAll('.manufacturing-fixture')[1].querySelector('select'); Object.getOwnPropertyDescriptor(HTMLSelectElement.prototype, 'value').set.call(control, 'body'); control.dispatchEvent(new Event('change', { bubbles: true })); })()`);
    await waitFor(window, `JSON.parse(window.__madcadGetSessionExport()).manufacturing.setups[0].fixtures[1].shape === 'body'`, 'szczęka CAD przed ponownym otwarciem');
    const serializedWithBodyFixture = await window.webContents.executeJavaScript(`window.__madcadGetSessionExport()`);
    await window.webContents.executeJavaScript(`window.__madcadVerifyLoadSerializedDocument(${JSON.stringify(serializedWithBodyFixture)})`);
    await waitFor(window, `window.__madcadVerifyEngineState?.status === 'ready' && window.__madcadVerifyDocumentState?.bodyIds?.length >= 2 && JSON.parse(window.__madcadGetSessionExport()).manufacturing.setups[0].fixtures[1].shape === 'body'`, 'ponowne otwarcie bryły mocowania');
    await window.webContents.executeJavaScript(`[...document.querySelectorAll('.workspace-tabs button')].find((button) => button.textContent.trim() === 'WYTWARZANIE').click()`);
    await waitFor(window, `window.__madcadManufacturingVisualState?.fixtureBodyMeshCount === 1 && document.querySelector('.manufacturing-fixture select')`, 'widok szczęki CAD po ponownym otwarciu');
    process.stdout.write(`${JSON.stringify({ screenshotPath, reportScreenshotPath, fixtureScreenshotPath, fixtureBodyScreenshotPath, gcodeScreenshotPath, gcodePath, gcodeBytes: Buffer.byteLength(gcode), programGcodePath, programGcodeBytes: Buffer.byteLength(programGcode), setupSheetPath, setupSheetBytes: Buffer.byteLength(setupSheet), profileBoundary: profileState, simulation: simulationState, reportVerified: true, ...state }, null, 2)}\n`);
  } catch (error) {
    exitCode = 1;
