...
app.whenReady().then(async () => {
  ...
  try {
    ...
    if (state.schemaVersion !== 17 || !state.setup?.id || state.activeSetupId !== state.setup.id || state.setup.stock.topOffset !== 5 || state.setup.operations?.length !== 3 || state.setup.operations[1].type !== 'pocket' || state.setup.operations[2].type !== 'contour' || !state.valid || state.validToolpaths !== 3 || !(state.toolpathSegments > 0) || !state.stockVisible || !state.insideViewport || state.horizontalOverflow) throw new Error(`Niepoprawny Setup CAM: ${JSON.stringify(state)}`);
