...
app.whenReady().then(async () => {
  ...
  try {
    ...
    const result = await window.webContents.executeJavaScript(`(() => { const value = window.__madcadVerifyDocumentState.command.beamFea.result; const panel = document.querySelector('.beam-fea-panel').getBoundingClientRect(); return { loadType: value.loadType, force: value.force, totalLoad: value.totalLoad, length: value.length, elements: value.elementCount, nodes: value.nodeCount, deflection: value.tipDeflection, error: value.convergenceError, reaction: value.reactionForce, moment: value.reactionMoment, limitations: value.limitations.length, distributedUnitVisible: document.querySelector('.beam-fea-panel')?.textContent.includes('N/mm'), diagramVisible: Boolean(document.querySelector('.beam-fea-chart polyline')), viewportNodes: window.__madcadBeamFeaVisualState?.nodeCount, deformationScale: window.__madcadBeamFeaVisualState?.scale, insideViewport: panel.left >= 0 && panel.top >= 0 && panel.right <= innerWidth && panel.bottom <= innerHeight, horizontalOverflow: document.documentElement.scrollWidth > innerWidth, scopeVisible: document.querySelector('.beam-fea-panel .analysis-scope')?.textContent.includes('nie MES dowolnej bryły 3D') }; })()`);
    await fs.writeFile(screenshotPath, (await window.webContents.capturePage()).toPNG());
    if (result.loadType !== 'distributed' || result.force !== 10 || Math.abs(result.totalLoad - result.force * result.length) > 1e-6 || result.elements !== 12 || result.nodes !== 13 || !(result.deflection > 0) || result.error > 1e-5 || Math.abs(result.reaction - result.totalLoad) > 1e-4 || Math.abs(result.moment - result.totalLoad * result.length / 2) > 1e-3 || result.limitations !== 3 || !result.distributedUnitVisible || !result.diagramVisible || result.viewportNodes !== 13 || !(result.deformationScale >= 1) || !result.insideViewport || result.horizontalOverflow || !result.scopeVisible) throw new Error(`Niepoprawny MES belki: ${JSON.stringify(result)}`);
