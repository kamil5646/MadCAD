...
app.whenReady().then(async () => {
  ...
  try {
    ...
    await setControl(window, '.beam-fea-panel input[type="text"]', '500', 0);
    await setControl(window, '.beam-fea-panel input[type="text"]', '37', 3);
    ...
    await setControl(window, '.beam-fea-factor-row > div input', '2', 2);
    await waitFor(window, `window.__madcadVerifyDocumentState.command.beamFea.result?.criticalLoadCase?.factor === 2`, 'własny współczynnik przeciążenia');
    await window.webContents.executeJavaScript(`document.querySelector('.beam-fea-factor-editor > header button').click()`);
    await waitFor(window, `document.querySelectorAll('.beam-fea-factor-row').length === 4`, 'dodany scenariusz obciążenia');
    await setControl(window, '.beam-fea-factor-row > input', 'Transport', 3);
    await setControl(window, '.beam-fea-factor-row > div input', '3', 3);
    await waitFor(window, `window.__madcadVerifyDocumentState.command.beamFea.result?.loadCases?.length === 4 && window.__madcadVerifyDocumentState.command.beamFea.result?.criticalLoadCase?.name === 'Transport'`, 'własny krytyczny scenariusz');
