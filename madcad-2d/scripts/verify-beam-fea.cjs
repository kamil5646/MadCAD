...
app.whenReady().then(async () => {
  ...
  try {
    ...
    const result = await window.webContents.executeJavaScript(`(() => { const value = window.__madcadVerifyDocumentState.command.beamFea.result; const panel = document.querySelector('.beam-fea-panel').getBoundingClientRect(); return { loadType: value.loadType, force: value.force, loadPositionPercent: value.loadPositionPercent, loadPosition: value.loadPosition, totalLoad: value.totalLoad, length: value.length, elements: value.elementCount, nodes: value.nodeCount, momentNodes: value.bendingMoments?.length, shearNodes: value.shearForces?.length, diagramMaximum: Math.max(...(value.bendingMoments || []).map((node) => node.moment)), shearMaximum: Math.max(...(value.shearForces || []).map((node) => node.shear)), deflection: value.tipDeflection, error: value.convergenceError, reaction: value.reactionForce, moment: value.reactionMoment, limitations: value.limitations.length, positionVisible: Array.from(document.querySelectorAll('.beam-fea-panel .command-field')).some((label) => label.textContent.includes('Położenie od utwierdzenia')), legendVisible: Boolean(document.querySelector('.beam-fea-legend')), chartTabsVisible: document.querySelectorAll('.beam-fea-chart-tabs button').length === 3, shearDiagramVisible: Boolean(document.querySelector('.beam-fea-shear-chart polyline')), viewportNodes: window.__madcadBeamFeaVisualState?.nodeCount, deformationScale: window.__madcadBeamFeaVisualState?.scale, viewportLoadPosition: window.__madcadBeamFeaVisualState?.loadPosition, viewportLoadPoint: window.__madcadBeamFeaVisualState?.loadPoint, loadArrowCount: window.__madcadBeamFeaVisualState?.loadArrowCount, support: window.__madcadBeamFeaVisualState?.support, insideViewport: panel.left >= 0 && panel.top >= 0 && panel.right <= innerWidth && panel.bottom <= innerHeight, horizontalOverflow: document.documentElement.scrollWidth > innerWidth, scopeVisible: document.querySelector('.beam-fea-panel .analysis-scope')?.textContent.includes('nie MES dowolnej bryły 3D') }; })()`);
    result.deflectionDiagramVisible = deflectionDiagramVisible;
    result.momentDiagramVisible = momentDiagramVisible;
    await fs.writeFile(screenshotPath, (await window.webContents.capturePage()).toPNG());
    if (result.loadType !== 'tip' || result.force !== 500 || result.loadPositionPercent !== 37 || Math.abs(result.loadPosition - result.length * 0.37) > 1e-6 || result.totalLoad !== 500 || result.elements !== 12 || result.nodes !== 13 || result.momentNodes !== 13 || result.shearNodes !== 24 || Math.abs(result.diagramMaximum - result.moment) > 1e-3 || Math.abs(result.shearMaximum - result.reaction) > 1e-3 || !(result.deflection > 0) || result.error > 1e-5 || Math.abs(result.reaction - 500) > 1e-4 || Math.abs(result.moment - result.force * result.loadPosition) > 1e-3 || result.limitations !== 3 || !result.positionVisible || !result.legendVisible || !result.chartTabsVisible || !result.deflectionDiagramVisible || !result.momentDiagramVisible || !result.shearDiagramVisible || result.viewportNodes !== 13 || Math.abs(result.viewportLoadPosition - result.loadPosition) > 1e-6 || result.viewportLoadPoint?.length !== 3 || result.loadArrowCount !== 1 || !result.support || !(result.deformationScale >= 1) || !result.insideViewport || result.horizontalOverflow || !result.scopeVisible) throw new Error(`Niepoprawny MES belki: ${JSON.stringify(result)}`);
