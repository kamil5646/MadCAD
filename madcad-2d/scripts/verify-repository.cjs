...
if (packageJson.version !== '6.4.6') throw new Error(`Wersja stabilna musi wynosić 6.4.6, jest ${packageJson.version}.`);
...
expectText(site, /paczki 6\.4\.6 są publikowane bez podpisu producenta/, 'ostrzeżenie o niepodpisanym wydaniu 6.4.6');
...
expectText(rootReadme, /Uwaga o wydaniu 6\.4\.6/, 'ostrzeżenie wydania w README');
...
expectText(appDialogs, /Wydanie 6\.4\.6 nie ma podpisu producenta/, 'ostrzeżenie o podpisie w aplikacji');
