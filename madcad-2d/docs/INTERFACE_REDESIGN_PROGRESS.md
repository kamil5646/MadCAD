...
- [x] Przepływ inspirowany Shapr3D: bezpośredni wybór płaszczyzny, adaptacyjne narzędzia po zaznaczeniu i zwijane filtry wyboru.
...
- 2026-08-30: na podstawie oficjalnego wzorca Shapr3D usunięto modalną zasłonę wyboru płaszczyzny. XY/XZ/YZ wybiera się nad działającym obszarem modelu, klawiszami `1–3` albo kliknięciem, a `Esc` anuluje bez utraty widoku.
- 2026-08-30: dodano adaptacyjną paletę po lewej stronie płótna. Dla profilu pokazuje przede wszystkim `Wyciągnij`, dla ściany modelowanie bezpośrednie, dla krawędzi zaokrąglenie/fazę, a dla bryły przesunięcie, obrót i szyk. Pozostałe poprawne operacje są w `Więcej`; stała wstążka nie zmienia kolejności.
- 2026-08-30: stały pięcioelementowy filtr wyboru zwinięto do jednego małego przycisku. Pełne filtry rozwijają się na żądanie i automatycznie podczas wyboru obszarem; wtedy działają skróty Shapr3D `B`, `F`, `E` oraz `Tab`. Stała instrukcja wyboru została ukryta wizualnie poza aktywną operacją, ale pozostała dostępna dla technologii asystujących.
- 2026-08-30: po przebudowie zaliczone `lint`, test komponentu wyboru płaszczyzny i adaptacyjnej palety, `build:ui`, `verify:interface-consistency` oraz `verify:assistive-tech`. Kontrola interfejsu potwierdza, że paleta nie wychodzi poza płótno, a wybór płaszczyzny nie jest modalny i zachowuje widoczność modelu.
- 2026-08-30: kontrola w działającej aplikacji wykryła, że profil zużyty przez `Wyciągnij` pozostawał nad bryłą i przechwytywał kliknięcia. Widoczność szkicu jest teraz wyłączana po powstaniu pierwszej operacji historii, chyba że użytkownik świadomie zaznaczy szkic lub ponownie wejdzie w jego edycję.
- 2026-08-30: końcowa regresja przeszła `lint`, 112/112 testów interfejsu, 186/186 testów rdzenia, `build:ui`, pełny `verify:modeling`, `verify:interface-consistency`, `verify:assistive-tech` i `verify:electron-security`. Test modelowania odrzuca ponownie sytuację, w której gotowy profil zasłania bryłę po wyciągnięciu.
