...
- [x] P1.35c Scena i render przeszła 203 testy core, 136 testów UI, pełny `verify:modeling` oraz osobny desktop E2E zmiany presetu, Undo/Redo i prawdziwego eksportu PNG. Lokalny build zastąpił `/Applications/MadCAD.app`, przeszedł `codesign --verify --deep --strict` i został sprawdzony przez interfejs macOS; SHA-256 `app.asar`: `1a38bb7e5f39200751f3d2d94e8721eaadd49defd4979c7c7b203137965d8cb8`. Poprzednia aplikacja: `~/.Trash/MadCAD-before-render-scene-20260905.app`. GitHub Release nie został utworzony.
- [x] P1.35d Naklejki na ścianach przeszły 204 testy core, 136 testów UI, pełny `verify:modeling` oraz desktop E2E dodania obrazu, edycji, usunięcia, Cofnij i czystego eksportu PNG. Lokalny build zastąpił `/Applications/MadCAD.app`, przeszedł `codesign --verify --deep --strict` i został sprawdzony w uruchomionej aplikacji przez interfejs macOS; źródłowy i zainstalowany `app.asar` mają SHA-256 `3dc1ba590326cc55d68d836aa0f4b6d9abe51243006d7dfdfc6471dce5839a5e`. Poprzednia aplikacja: `~/.Trash/MadCAD-before-decals-20260905.app`. GitHub Release nie został utworzony.
- [x] P1.35e Storyboard animacji przeszedł 205 testów core, 137 testów UI, pełny `verify:modeling` oraz desktop E2E tworzenia dwóch klatek, rzeczywistego odtwarzania i Undo/Redo. Lokalny build zastąpił `/Applications/MadCAD.app`, przeszedł `codesign --verify --deep --strict` i uruchomił się jako świeży proces; źródłowy i zainstalowany `app.asar` mają SHA-256 `ac250d0c4f0b36b527867885402bc0058d6ea40098ebfe239c7d17cdbd3f0a7f`. Poprzednia aplikacja: `~/.Trash/MadCAD-before-storyboard-20260905.app`. GitHub Release nie został utworzony.
- [x] P1.35f Ruch części i kamery storyboardu przeszedł 205 testów core, 137 testów UI, pełny `verify:modeling` oraz desktop E2E przesunięcia wystąpienia XYZ, zapisu kamery, notatki, odtwarzania i Undo/Redo. Lokalny build zastąpił `/Applications/MadCAD.app`, przeszedł `codesign --verify --deep --strict` i uruchomił się jako świeży proces; źródłowy i zainstalowany `app.asar` mają SHA-256 `dde7369e4cb7b3d2e5fcc403f36cd1ae60b2a0d1f7a619944b70c80e6d118249`. Poprzednia aplikacja: `~/.Trash/MadCAD-before-storyboard-motion-20260907.app`. GitHub Release nie został utworzony.
- [x] P1.35g Obrót części w storyboardzie przeszedł 205 testów core, 137 testów UI, pełny `verify:modeling` oraz desktop E2E zapisu i rzeczywistego odtworzenia obrotu 45° razem z przesunięciem. Panel został sprawdzony wizualnie i rozdziela przesunięcie w milimetrach od obrotu w stopniach bez przepełnienia. Lokalny build zastąpił `/Applications/MadCAD.app`, przeszedł `codesign --verify --deep --strict` i został uruchomiony jako świeży proces z zasobu `/Applications/MadCAD.app/Contents/Resources/app.asar/dist/index.html`; źródłowy i zainstalowany `app.asar` mają SHA-256 `d6382f3f0fb18bffa4f1b79e8bff54aa2b59f0b8ef186cd5884d9597ccb8d7a1`. Poprzednia aplikacja: `~/.Trash/MadCAD-before-storyboard-rotation-20260907.app`. GitHub Release nie został utworzony.
- [x] P1.35h Graficzne prowadnice animacji przeszły 205 testów core, 137 testów UI, pełny `verify:modeling` i desktop E2E potwierdzający turkusową strzałkę przesunięcia oraz bursztynowy łuk obrotu dla wybranego wystąpienia. Zrzut `artifacts/madcad-storyboard.png` sprawdzono wizualnie po pogrubieniu geometrii i zakotwiczeniu jej w środku bryły. Lokalny build zastąpił `/Applications/MadCAD.app`, przeszedł `codesign --verify --deep --strict` i po zakończeniu wszystkich starszych procesów uruchomił się poprawnie jako jeden świeży proces PID `35475`; źródłowy i zainstalowany `app.asar` mają SHA-256 `320065537df43686d7aae05b836ec19681d67c834465ab36ae2c44b38544830d`. Poprzednia aplikacja: `~/.Trash/MadCAD-before-storyboard-guides-20260907.app`. GitHub Release nie został utworzony.
- [x] P1.35i Animacja wartości jointów przeszła 205 testów core, 137 testów UI, pełny `verify:modeling` oraz desktop E2E ustawienia 55° z panelu, zapisu klatki i użycia wartości przez renderer bez zmiany projektowego jointa 35° i aktywnej konfiguracji. Lokalny build zastąpił `/Applications/MadCAD.app`, przeszedł `codesign --verify --deep --strict` i został potwierdzony w uruchomionym oknie z zasobu `/Applications/MadCAD.app/Contents/Resources/app.asar/dist/index.html`; źródłowy i zainstalowany `app.asar` mają SHA-256 `baf2b0404b34ecf8405096d9bd96ac636d0478adec23ba2bc124bdf079423668`. Poprzednia aplikacja: `~/.Trash/MadCAD-before-storyboard-joints-20260907.app`. GitHub Release nie został utworzony.
- [x] P1.35j Eksport storyboardu przeszedł 206 testów core, 137 testów UI, pełny `verify:modeling` oraz desktop E2E rzeczywistego zapisu filmu WebM 24 fps i drukowalnej instrukcji HTML z opisem, ruchem, obrotem i wartością jointa. Lokalny build zastąpił `/Applications/MadCAD.app`, przeszedł `codesign --verify --deep --strict` i został potwierdzony jako świeży proces PID `38982` z zasobu `/Applications/MadCAD.app/Contents/Resources/app.asar/dist/index.html`; źródłowy i zainstalowany `app.asar` mają SHA-256 `538b133cbb664ec81ef3389906b644c450d1bd20f80a6eaa808dd49c3d553f39`. Poprzednia aplikacja: `~/.Trash/MadCAD-before-storyboard-export-20260907-1400.app`. GitHub Release nie został utworzony.
- [x] P1.35k Widoki klatek w instrukcji przeszły 206 testów core, 137 testów UI, pełny `verify:modeling` i desktop E2E wymagający dokładnie dwóch prawdziwych obrazów PNG osadzonych w HTML oraz zachowania stanu animacji po eksporcie. Lokalny build zastąpił `/Applications/MadCAD.app`, przeszedł `codesign --verify --deep --strict` i został potwierdzony w świeżym procesie PID `39708` z zasobu `/Applications/MadCAD.app/Contents/Resources/app.asar/dist/index.html`; źródłowy i zainstalowany `app.asar` mają SHA-256 `95adc57c8af7b526cd302d5e557a894b64d28d76da4f986bbfa0b94c5e3f6a67`. Poprzednia aplikacja: `~/.Trash/MadCAD-before-storyboard-frame-images-20260907-1410.app`. GitHub Release nie został utworzony.
- [x] P1.36a Szybka analiza statyczna przeszła 207 testów core, 139 testów UI, pełny `verify:modeling` oraz desktop E2E zmiany materiału i siły, walidacji osi, wyników liczbowych, jawnego zakresu i zmieszczenia panelu. Zrzut `artifacts/madcad-static-screening.png` sprawdzono wizualnie. Lokalny build zastąpił `/Applications/MadCAD.app`, przeszedł `codesign --verify --deep --strict` i został potwierdzony w świeżym procesie PID `41841` z zasobu `/Applications/MadCAD.app/Contents/Resources/app.asar/dist/index.html`; źródłowy i zainstalowany `app.asar` mają SHA-256 `af89d5fd8097a5bed9ce377382851f700bbb3c8d9ecfacbc7b9b4bc5a8ff8672`, a nagłówek integralności `c91aba34ed4d3710304b0b15e369457a45d6d2ba3216bd922ff6db2bcf006cbf`. Poprzednia aplikacja: `~/.Trash/MadCAD-before-static-screening-20260907-2010.app`. GitHub Release nie został utworzony.
- [x] P1.36b Szybka analiza cieplna przeszła 208 testów core, 141 testów UI, pełny `verify:modeling` oraz desktop E2E zmiany materiału i temperatur, walidacji równych temperatur, wyników liczbowych, ostrzeżenia materiałowego i zmieszczenia panelu. Po kontroli zrzutu poprawiono zbyt wąskie pola temperatur, a końcowy `artifacts/madcad-thermal-screening.png` sprawdzono wizualnie. Lokalny build zastąpił `/Applications/MadCAD.app`, przeszedł `codesign --verify --deep --strict` i uruchomił się jako świeży proces PID `42855`; źródłowy i zainstalowany `app.asar` mają SHA-256 `76574b9929cec8bc1446f36d10cd694d542dd81d3f675e8007ca8407b849ec22`, a nagłówek integralności `d3647e0d752c7e62fafbf7792359337664361c151aba755389108339ece81e9e`. Poprzednia aplikacja: `~/.Trash/MadCAD-before-thermal-screening-20260907-2050.app`. GitHub Release nie został utworzony.
- [x] P1.36c MES belki 1D przeszedł 209 testów core, 143 testy UI, pełny `verify:modeling` oraz desktop E2E zmiany siły i liczby elementów, walidacji siatki, reakcji podpory i zgodności z rozwiązaniem analitycznym. Panel bez przepełnienia sprawdzono wizualnie w `artifacts/madcad-beam-fea.png`. Lokalny build zastąpił `/Applications/MadCAD.app`, przeszedł `codesign --verify --deep --strict` i został potwierdzony przez interfejs macOS jako świeży proces PID `44432` z zasobu `/Applications/MadCAD.app/Contents/Resources/app.asar/dist/index.html`; źródłowy i zainstalowany `app.asar` mają SHA-256 `d118da6a753b8872b388ad2cd2bbf6d4245e0b543d9dc77019a0b7dff546fa72`, a nagłówek integralności `34cb17d6e3e9cad0079424eb7553efda1b7cc1a459aa5d2f0eaa6bd1979f578f`. Poprzednia aplikacja: `~/.Trash/MadCAD-before-beam-fea-20260908-0745.app`. GitHub Release nie został utworzony.
- [x] P1.36d Wizualizacja MES pokazuje wykres ugięcia w panelu oraz kolorowaną linię deformacji i wszystkie węzły bezpośrednio na bryle. Przeszła 209 testów core, 143 testy UI, pełny `verify:modeling` i desktop E2E wymagający 13 węzłów w rendererze, automatycznej skali oraz diagramu SVG. Końcowy `artifacts/madcad-beam-fea.png` sprawdzono wizualnie, a panel ograniczono do wysokości obszaru roboczego z przewijaniem treści. Lokalny build zastąpił `/Applications/MadCAD.app`, przeszedł `codesign --verify --deep --strict` i został potwierdzony przez interfejs macOS jako świeży proces PID `45495` z zasobu `/Applications/MadCAD.app/Contents/Resources/app.asar/dist/index.html`; źródłowy i zainstalowany `app.asar` mają SHA-256 `3161cd3cfeed4c7ae35b39203486dc1649cff3a8cb6cfcc5fbbc64e080698e9c`, a nagłówek integralności `f0e2c84dab55eee09379349ec0ccf4794ad55e8139ac9a503b509d6c512bad44`. Poprzednia aplikacja: `~/.Trash/MadCAD-before-beam-fea-visual-20260908-0755.app`. GitHub Release nie został utworzony.
- [x] P1.36e Obciążenie rozłożone MES przeszło 210 testów core, 144 testy UI, pełny `verify:modeling`, `verify:package -- mac` oraz desktop E2E sił węzłowych, reakcji, momentu `qL²/2`, ugięcia analitycznego, jednostki N/mm, wykresu i zmieszczenia panelu. Zrzut `artifacts/madcad-beam-fea.png` sprawdzono wizualnie. Produkcyjne paczki pomijają zbędne mapy źródłowe zależności, co skraca kolejne buildy. Lokalny build bez certyfikatu dystrybucyjnego zastąpił `/Applications/MadCAD.app`, po usunięciu wyłącznie metadanych rozszerzonych otrzymał podpis ad-hoc, przeszedł `codesign --verify --deep --strict` i został potwierdzony przez interfejs macOS jako świeży proces PID `52961` z zasobu `/Applications/MadCAD.app/Contents/Resources/app.asar/dist/index.html`; źródłowy i zainstalowany `app.asar` mają SHA-256 `39d6f5a19591cb6bd1325bda533eb67fc0f813a1eb4e5d6e4372f9f118b7cbbd`, a nagłówek integralności `8b70a08eed0fdfe2d88f8f2c74a6cbf290aa2acb12493acc1630dc04f21f5c8c`. Poprzednia aplikacja: `~/.Trash/MadCAD-before-beam-udl-20260908-0820.app`. GitHub Release nie został utworzony.
- [x] P1.36f Dowolne położenie siły skupionej przeszło 211 testów core, 145 testów UI, lint, pełny `verify:modeling` w 78,177 s oraz desktop E2E obciążenia w 37% długości, funkcji kształtu, reakcji, momentu, ugięcia analitycznego, wykresu i znacznika miejsca przyłożenia w rendererze. Pierwszy pełny pomiar z równolegle otwartą aplikacją przekroczył budżet, dlatego został uczciwie odrzucony i powtórzony na czysto. Końcowy `artifacts/madcad-beam-fea.png` sprawdzono wizualnie. Lokalny build bez certyfikatu dystrybucyjnego zastąpił `/Applications/MadCAD.app`, otrzymał podpis ad-hoc, przeszedł `codesign --verify --deep --strict` i po zamknięciu starszych procesów został potwierdzony przez interfejs macOS jako jeden świeży proces PID `54316` z zasobu `/Applications/MadCAD.app/Contents/Resources/app.asar/dist/index.html`; źródłowy i zainstalowany `app.asar` mają SHA-256 `7acac4f4336712ae6ef33d1077b3f7d69083c0544ca95adfa91987736d787d55`, a nagłówek integralności `77b45565bc81e65f1c548b82ac81a608b19635fba769deb5a3e63d3a20277e5b`. Poprzednia aplikacja: `~/.Trash/MadCAD-before-beam-load-position-20260908-0831.app`. GitHub Release nie został utworzony.
- [x] P1.36g Symbole warunków brzegowych przeszły 211 testów core, 145 testów UI, lint, pełny `verify:modeling` w 76,935 s oraz desktop E2E wymagający czerwonego utwierdzenia, jednej strzałki siły skupionej, sześciu strzałek obciążenia rozłożonego i zgodnej legendy. Końcowy `artifacts/madcad-beam-fea.png` sprawdzono wizualnie. Lokalny build bez certyfikatu dystrybucyjnego zastąpił `/Applications/MadCAD.app`, otrzymał podpis ad-hoc, przeszedł `codesign --verify --deep --strict` i został potwierdzony przez interfejs macOS jako świeży proces PID `56102` z zasobu `/Applications/MadCAD.app/Contents/Resources/app.asar/dist/index.html`; źródłowy i zainstalowany `app.asar` mają SHA-256 `c2f4784e7e5d413b299b0ea11a4029c872f61045dd9058a02bed53d9fccc9526`, a nagłówek integralności `1d74c9bbd2315199c859865537f9f7e1600f556f1cf91a530aca53c5e6212e3f`. Poprzednia aplikacja: `~/.Trash/MadCAD-before-beam-boundary-symbols-20260908-0852.app`. GitHub Release nie został utworzony.
- [x] P1.36h Diagram momentu zginającego przeszedł 211 testów core, 145 testów UI, lint, pełny `verify:modeling` w 75,982 s oraz desktop E2E wymagający 13 punktów diagramu, zgodności jego maksimum z momentem podpory i widocznego SVG bez przepełnienia panelu. Końcowy `artifacts/madcad-beam-fea.png` sprawdzono wizualnie. Lokalny build bez certyfikatu dystrybucyjnego zastąpił `/Applications/MadCAD.app`, otrzymał podpis ad-hoc, przeszedł `codesign --verify --deep --strict` i został potwierdzony przez interfejs macOS jako świeży proces PID `56661` z zasobu `/Applications/MadCAD.app/Contents/Resources/app.asar/dist/index.html`; źródłowy i zainstalowany `app.asar` mają SHA-256 `bc6a0013f6d3fab0e3b974639b79e06f571f351d6c06c00423e1da921838f0ae`, a nagłówek integralności `416d757db05a2ef184fa9c1d2ecf9ea05e2b1cabef6d48437fb03520a28b62b9`. Poprzednia aplikacja: `~/.Trash/MadCAD-before-beam-moment-diagram-20260908-0900.app`. GitHub Release nie został utworzony.
- [x] P1.36i Diagram siły tnącej i zwarty przełącznik Ugięcie/Moment/Tnąca przeszły 211 testów core, 145 testów UI, lint, pełny `verify:modeling` w 77,774 s oraz desktop E2E wymagający 24 wartości końcowych siły tnącej dla 12 elementów, zgodności maksimum z reakcją podpory, działania wszystkich trzech wykresów i braku przepełnienia panelu. Zrzut `artifacts/madcad-beam-fea.png` sprawdzono wizualnie. Lokalny build bez certyfikatu dystrybucyjnego zastąpił `/Applications/MadCAD.app`, otrzymał podpis ad-hoc, przeszedł `codesign --verify --deep --strict` i został potwierdzony przez interfejs macOS jako jeden świeży proces PID `57244` z zasobu `/Applications/MadCAD.app/Contents/Resources/app.asar/dist/index.html`; źródłowy i zainstalowany `app.asar` mają SHA-256 `04c38f637473ec87fcd4a53aee0b8338b8a8f0b90a505ca5befdafdbdc79c9d0`, a nagłówek integralności `896bb6322768a57022701dcc39cfa060cdf2c446cce232c5b85dc9790382a5e2`. Poprzednia aplikacja: `~/.Trash/MadCAD-before-beam-shear-diagram-20260908-0907.app`. GitHub Release nie został utworzony.
- [x] P1.36j Diagram naprężenia zginającego przeszedł 211 testów core, 145 testów UI, lint, pełny `verify:modeling` w 77,364 s oraz desktop E2E wymagający 13 wartości naprężenia, zgodności maksimum wykresu z wynikiem bezpieczeństwa, działania czterech przełączanych wykresów i braku przepełnienia panelu. Zrzut `artifacts/madcad-beam-fea.png` sprawdzono wizualnie. Lokalny build bez certyfikatu dystrybucyjnego zastąpił `/Applications/MadCAD.app`, otrzymał podpis ad-hoc, przeszedł `codesign --verify --deep --strict` i został potwierdzony przez interfejs macOS jako jeden świeży proces PID `57766` z zasobu `/Applications/MadCAD.app/Contents/Resources/app.asar/dist/index.html`; źródłowy i zainstalowany `app.asar` mają SHA-256 `8b5c63cdeb4fe5d302c6611ee1b06b23d0297fe2e043b3eb00629281b3f8ec61`, a nagłówek integralności `e24ef0391a884b6f1adea7ee93185452e72886bddffc08f1e28fb41479092f40`. Poprzednia aplikacja: `~/.Trash/MadCAD-before-beam-stress-diagram-20260908-0914.app`. GitHub Release nie został utworzony.
- [x] P1.36k Mapa wykorzystania granicy plastyczności przeszła 211 testów core, 145 testów UI, lint, pełny `verify:modeling` w 80,196 s oraz desktop E2E sprawdzający zgodność procentu wykorzystania z naprężeniem i materiałem, bezpieczne zielone punkty, trzy węzły krytyczne i czerwone znaczniki po kontrolowanym zwiększeniu obciążenia oraz powrót do stanu bezpiecznego. Lokalny build bez certyfikatu dystrybucyjnego zastąpił `/Applications/MadCAD.app`, otrzymał podpis ad-hoc, przeszedł `codesign --verify --deep --strict` i został potwierdzony przez interfejs macOS jako jeden świeży proces PID `58797` z zasobu `/Applications/MadCAD.app/Contents/Resources/app.asar/dist/index.html`; źródłowy i zainstalowany `app.asar` mają SHA-256 `daa88a37a101256c2fb70588a5115a959bdf41a047f1b8542983032722e77059`, a nagłówek integralności `eff55ee68c1246a1eb6861919d00352967cb66ec12dde952f04f67f46a60fce3`. Poprzednia aplikacja: `~/.Trash/MadCAD-before-beam-yield-map-20260908-0923.app`. GitHub Release nie został utworzony.
- [x] P1.36l Konfigurowalny cel bezpieczeństwa 1–10 przeszedł 211 testów core, 145 testów UI, lint, pełny `verify:modeling` w 77,555 s oraz desktop E2E sprawdzający domyślny cel 2, spełnienie celu, matematyczną zgodność procentowego marginesu i zachowanie mapy przekroczeń. Lokalny build bez certyfikatu dystrybucyjnego zastąpił `/Applications/MadCAD.app`, otrzymał podpis ad-hoc, przeszedł `codesign --verify --deep --strict` i został potwierdzony przez interfejs macOS jako jeden świeży proces PID `59868` z zasobu `/Applications/MadCAD.app/Contents/Resources/app.asar/dist/index.html`; źródłowy i zainstalowany `app.asar` mają SHA-256 `5bdbaeaaa45bf12af31e4adf819fd5bc03f6fa4494e59efe5e33f282bfc31495`, a nagłówek integralności `80ca1ebea939a449dc49ed45cbc9183bd21422054de761e147dcdacc46b8ea4b`. Poprzednia aplikacja: `~/.Trash/MadCAD-before-beam-safety-target-20260908-0931.app`. GitHub Release nie został utworzony.
- [x] P1.36m Kombinacja siły skupionej i rozłożonej przeszła 212 testów core, 146 testów UI, lint, pełny `verify:modeling` w 77,529 s oraz desktop E2E sprawdzający superpozycję 500 N w 37% długości i 10 N/mm, sumę reakcji, momentów, ugięcie analityczne, punkt przyłożenia i siedem strzałek obciążenia. Lokalny build bez certyfikatu dystrybucyjnego zastąpił `/Applications/MadCAD.app`, otrzymał podpis ad-hoc, przeszedł `codesign --verify --deep --strict` i został potwierdzony przez interfejs macOS jako jeden świeży proces PID `60903` z zasobu `/Applications/MadCAD.app/Contents/Resources/app.asar/dist/index.html`; źródłowy i zainstalowany `app.asar` mają SHA-256 `3c43abec178698ea9ceb00a0b563a9261a411c59f75fb6db3e3d7eda530729ce`, a nagłówek integralności `2df1aef8295cb44d5f028eaf56ab4e9cfe7094b55628914071ad176f26fdd568`. Poprzednia aplikacja: `~/.Trash/MadCAD-before-combined-beam-load-20260908-0940.app`. GitHub Release nie został utworzony.
