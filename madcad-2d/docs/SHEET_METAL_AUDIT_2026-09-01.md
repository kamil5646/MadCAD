...
## Ukończony zakres: baza i kołnierz krawędziowy
...
- dowód wizualny gotowej bryły jest zapisany w `artifacts/madcad-sheet-metal-flange.png`.

## Dalsza kolejność

1. Hem i Rip z jawną szczeliną technologiczną.
2. Unfold/Refold oraz flat pattern liczony z promienia, grubości i współczynnika K.
3. Skojarzona tabela gięć dla arkusza 2D.
...
Po przejściu pełnej walidacji build arm64 z bazą i kołnierzem zastępuje `/Applications/MadCAD.app`, otrzymuje lokalny podpis ad-hoc i przechodzi `codesign --verify --deep --strict`. GitHub pozostaje bez zmian do wyraźnego polecenia wydania.
