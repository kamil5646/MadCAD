...
test('Form wygładza zamkniętą klatkę Catmulla-Clarka i przygotowuje konwersję B-Rep', () => {
  ...
  const translatedEdge = translateFormControlPoints([], [4, 5], 2, 6, 'x');
  assert.deepEqual(translatedEdge[4], [0, 0, 6]);
  assert.deepEqual(translatedEdge[5], [0, 0, 6]);
  ...
  const edgeLoopCage = insertFormEdgeLoop(createBoxControlCage(40, 30, 20, [], [8]), FORM_CONTROL_EDGES[8], 0.25);
  assert.equal(edgeLoopCage.vertices.length, 12);
  assert.equal(edgeLoopCage.faces.length, 10);
  assert.equal(formControlEdges(edgeLoopCage.faces).length, 20);
  assert.equal(edgeLoopCage.insertedVertexIndexes.length, 4);
  assert.deepEqual(edgeLoopCage.vertices[8], [20, -15, -5]);
  assert.equal(edgeLoopCage.creaseEdges.length, 2);
  const edgeLoopMesh = createRoundedBoxFormMesh({ width: 40, depth: 30, height: 20, subdivisions: 1, creaseEdges: [8], insertEdge: { enabled: true, edgeIndex: 8, position: 0.25 } });
  ...
  assert.equal(edgeLoopReport.nonManifoldEdges, 0);
  ...
  const fillSourceCage = bridgeFormFaces(insertFormEdgeLoop(createBoxControlCage(40, 30, 20), FORM_CONTROL_EDGES[4], 0.5), 6, 9, 0.45);
  const fillSourcePairs = formControlSymmetryPairs({ enabled: true, edgeIndex: 4, position: 0.5 }, 'x', { enabled: true, firstFaceIndex: 6, secondFaceIndex: 9, inset: 0.45 });
  const fillFaceIndexes = symmetricFormFaceIndexes(fillSourceCage, fillSourcePairs, 0);
  assert.deepEqual(fillFaceIndexes, [0, 1]);
  const filledCage = fillFormHoles(fillSourceCage, fillFaceIndexes, [[0, 0, 1], [0, 0, 1]]);
  assert.equal(filledCage.vertices.length, 22);
  assert.equal(filledCage.faces.length, 26);
  const filledMesh = createRoundedBoxFormMesh({ width: 40, depth: 30, height: 20, subdivisions: 1, insertEdge: { enabled: true, edgeIndex: 4, position: 0.5 }, bridge: { enabled: true, firstFaceIndex: 6, secondFaceIndex: 9, inset: 0.45 }, fillHole: { enabled: true, faceIndexes: fillFaceIndexes }, fillHoleOffsets: [[0, 0, 1], [0, 0, 1]] });
  ...
  assert.equal(filledReport.inconsistentEdges, 0);
  const filledSymmetryPairs = formControlSymmetryPairs({ enabled: true, edgeIndex: 4, position: 0.5 }, 'x', { enabled: true, firstFaceIndex: 6, secondFaceIndex: 9, inset: 0.45 }, { enabled: true, faceIndex: 0 });
  assert.equal(filledSymmetryPairs[20], 21);
  assert.equal(filledSymmetryPairs[21], 20);
  ...
  const creased = createRoundedBoxFormMesh({ width: 40, depth: 30, height: 20, subdivisions: 2, creaseEdges: [0, 1, 2, 3] });
  assert.deepEqual(creased.creaseEdges, [0, 1, 2, 3]);
  assert.notDeepEqual(creased.vertices, mesh.vertices);
  const creasedReport = inspectMesh(creased);
  assert.equal(creasedReport.boundaryEdges, 0);
  assert.equal(creasedReport.nonManifoldEdges, 0);
  assert.equal(creasedReport.inconsistentEdges, 0);
  const firstCreaseLevel = subdivideCatmullClark(createBoxControlCage(40, 30, 20, [], [0]));
  assert.equal(firstCreaseLevel.creaseEdges.length, 2);
  ...
  assert.deepEqual(prepared.fillHole, { enabled: true, faceIndex: 0, faceIndexes: [0, 1] });
  assert.equal(prepared.fillHoleOffsets.length, 2);
  ...
  const invalidCrease = structuredClone(document);
  invalidCrease.features[0].creaseEdges = [0, 0, 12];
  const invalidCreaseResult = validateDocument(invalidCrease);
  assert.equal(invalidCreaseResult.valid, false);
  assert.ok(invalidCreaseResult.issues.some((issue) => issue.path.endsWith('.creaseEdges')));

  const invalidEdgePosition = structuredClone(document);
  invalidEdgePosition.parameters[2].expression = '0.25';
  assert.throws(() => prepareDocument(invalidEdgePosition), /osi symetrii/);
});
...
test('okna Electron i preload utrzymują sandbox oraz jedną bramę IPC', async () => {
  ...
  assert.equal((mainSource.match(/registerTrustedIpcHandler\('madcad:/g) || []).length, 27);
  assert.match(preloadSource, /openProjectFile/);
  assert.match(preloadSource, /packAndGoProject/);
  assert.match(preloadSource, /licenseGetStatus[\s\S]*licenseLogin[\s\S]*licenseRegister[\s\S]*licenseStartTrial[\s\S]*licenseLogout[\s\S]*licenseRequestPasswordReset[\s\S]*licenseResetPassword/);
